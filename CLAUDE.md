...
3. ~~**画面から CSV をアップロードできるようにする**~~ — 完了
   ...
   失敗時は `LoadError` を日本語メッセージで画面表示）
   - ~~**ヘッダ行の指定を目視で選べるようにする**~~ — 完了
     （`data.preview_upload()` で先頭行を型解釈なしにプレビューし、
     `data.guess_header_row()` で初期選択を推定。最終判断はプレビュー表の
     行クリックに一本化し、推定が外れても手動で選び直せば成立する設計）
