...
**散布図は 50,000 行を境に、点表示と密度表示を自動で切り替える**
点表示は 10 万行でドラッグ応答が 518ms、100 万行で 6.8 秒と非線形に悪化する一方、
密度表示（raster）は 1 万〜100 万行でほぼ一定だった（`docs/performance.md`）。
...
- [ ] リサイズへの追従（現在はファイル読み込み・軸変更時の幅で描画）
- [ ] Safari / Firefox での動作・性能の検証（計測はヘッドレス Chromium のみ）
- [ ] 1万〜10万行の間の計測（dot/raster の閾値 50,000 は暫定値）
- [ ] Playwright によるブラウザ駆動の自動テスト
