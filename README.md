...
| 自動の初期表示 | 相関が最も強い2列の散布図とそのヒストグラム、群間の差が最も大きいカテゴリ×数値の誤差棒を自動で並べる |
| グラフ | 散布図・ヒストグラム・棒グラフ（件数 / 平均）・平均±誤差棒（標準誤差 / 標準偏差）・折れ線に加え、統計用に Q-Q プロット・バイオリン図・回帰の残差プロット・相関行列（セルのクリックで散布図の軸を切り替え）・散布図行列。選んだ列の型から描けるものだけを選択肢に出し、複数並べて追加・削除できる |
| クロスフィルタ | 数値の軸はドラッグ、カテゴリの軸はクリックで選択し、すべてのグラフと統計量が連動。母集団（灰）と選択中（色）を重ねて表示 |
...
| 重回帰 | 目的変数1列を複数の数値列で説明する式を、母集団・選択中の両方で当てはめて並べる。係数・標準化係数 β・p 値・VIF・R²（調整済み）と、選択で効き方が変わった列の要約。他の列で表せる列は理由を示して外す |
| 機械学習 | k-means（クラスタ数は自動）、PCA、ランダムフォレストの変数重要度、階層クラスタリング（Ward / 平均 / 完全連結、z-score 標準化、クラスタ付きヒートマップ、選択範囲だけで実行も可）。クラスタと主成分得点は列として追加され、色分け・軸・絞り込み・選択に使える |
...
| 可視化・連動 | Mosaic (`@uwdata/vgplot`、`@uwdata/mosaic-plot`) | チャート間の選択連動（`Selection.crossfilter`）と SQL 集計の橋渡し。mosaic-plot は raster の不具合への対処で直接参照 |
| ファイル解析 | SheetJS (`xlsx`) | CSV / Excel を同じ経路で2次元配列にする |
| 統計 | jStat | t 分布・χ² 分布・F 分布の累積分布関数（p 値）。平均・分散・共分散の集計自体は DuckDB 側 |
| 機械学習 | 自前実装 + ml-hclust（Web Worker） | k-means・PCA・ランダムフォレストは自前、階層クラスタリングは ml-hclust。別スレッドで計算し、操作を止めない |
| 統計グラフ | Observable Plot | vgplot にマークの無い相関行列・Q-Q・バイオリン図の描画。集計と連動は Mosaic 経由 |
...
web/src/
  ...
  charts.ts        各グラフの組み立て（dot/raster 自動切替、回帰直線）
  chartTypes.ts    列の型から選べるグラフの種類と、初期表示の自動決定
  chartGrid.ts     グラフのカードの並び（追加・削除・設定変更）
  statCharts.ts    相関行列・Q-Q・バイオリン図（Plot で描き、選択は $brush に書く）
  hclustPanel.ts   階層クラスタリング（樹形図・クラスタ付きヒートマップ）
  ...
  multiRegression.ts 重回帰のタブ（平均と共分散を SQL で集計し、係数は JS で解く）
...
web/e2e/           ブラウザ駆動テスト（選択の連動・グラフの種類・統計グラフ・階層クラスタ・絞り込み・書き出し・重回帰など41件）
...
**グラフの種類は、連動に参加できるものだけ**
どのグラフも $brush / $filter に参加し、数値の軸はドラッグ、カテゴリの軸はクリックで
選ぶ。X・Y の列の型から描けるグラフだけを選択肢に出すので、意味のない組み合わせは
作れない。箱ひげ図は、vgplot に対応するマークが無く、ひげの端（四分位範囲の
1.5倍以内で最も外側の値）が集計関数1つでは求まらないため、入れていない。

**ドラッグしたグラフ自身でも、選んだ点に色を付ける**
crossfilter は、自分のグラフで選んだ条件を自分には効かせない（効かせると選択外が消え、
選び直せなくなるため）。そのため散布図で囲んでも自分の点の色は変わらず、枠でしか選択が
分からなかった。散布図は「母集団（灰）」と「選択中（色）」の2層なので、色の層だけを
自分の選択も含む条件（$selected）で絞れば、選択外は灰色で残したまま枠の中だけ色が付く。
カテゴリの棒グラフで先に取った方法と同じで、どのグラフで選んでも見え方がそろう。
ドラッグ中に散布図自身も描き直すことになるが、点で描くのは 15,000 行未満だけで、
その範囲では1フレーム 100ms 未満に収まることを測って確かめた（`docs/performance.md`）。

**重回帰は、行を取り出さず平均と共分散だけを集計して解く**
最小二乗の係数は X'X と X'y だけで決まり、どちらも平均と共分散から組み立てられる。
そこで DuckDB では全ペアの共分散だけを1クエリで集計し、説明変数の数ほどの大きさの行列を
JS で解いている。行数が増えても JS の計算量は変わらず、ライブラリも増やさずに済んだ。
他の列の和で表せる列（完全な多重共線性）は、解く前に見つけて理由を示して外す。
グラフの種類ではなくタブにしたのは、式や係数の表はグラフ上の選択（$brush）に参加できず、
グラフの種類を増やす条件に合わないため。その代わり選択中の行でも当てはめ、母集団と並べる。

**vgplot に無い図も、選択の仕組みは vgplot と同じにする**
相関行列・Q-Q プロット・バイオリン図・樹形図は vgplot にマークが無いので、Observable Plot や
SVG で描いた。ただし集計は Mosaic のクライアントで母集団・選択中それぞれに投げ、グラフ上の
クリックやドラッグは vgplot の範囲選択と同じ形の「節」にして $brush に書き込む。こうすると、
自作の図で選んでも他のすべてのグラフと統計量が連動し、選択の表示や解除の仕組みもそのまま使える。

**階層クラスタリングは 800 行まで。超えたら間引き、何件から何件にしたかを出す**
結合の手順が行数のほぼ3乗で重くなる。「実行」から結果まで2秒以内を目安に、Chromium・WebKit・
Firefox × 3つの連結法で測って決めた（`docs/performance.md`）。Node.js だけで測ったときは
1,000 行で約1.2秒だったが、Firefox では 2.2〜2.7 秒かかったため、800 行に下げた。
...
**vgplot の raster は、選択の結果が0行だと落ちていた**
散布図行列（点の総数が多いと密度表示になる）で、2つのグラフの選択が重ならず0行になると、
raster マークの色の割り当てが空の結果で例外になり、直前の選択の画像が残って見えていた。
0行のときだけ空の画像にする対処を入れた（`rasterPatch.ts`）。

**自作のグラフで、別のグラフを選んだ直後のドラッグが消えていた**
自作のグラフは集計が届くたびに図を作り直すため、ドラッグの最中に図が差し替わって
ドラッグが失われていた。ドラッグ中は描き直しを後回しにした。

## 今後

- [ ] Safari / Firefox での性能計測（動作はテストで確認済み。点表示/密度表示の閾値
  15,000 は Chromium での計測に基づく）
- [x] 散布図そのものをドラッグしたときの、自チャート内での選択の強調
  （点表示のとき。密度表示では背景の層が無いため、従来どおり枠で示す）
